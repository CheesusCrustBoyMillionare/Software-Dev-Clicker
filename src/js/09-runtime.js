  ...
  for (let n = 1; n <= 3; n++) {
    ...
    if (data) {
      ...
      if (data.v >= 2 && data.careerStarted && data.founder) {
        ...
        // v11: show bankruptcy count for roguelite saves (💀 is the game's
        // established bankruptcy icon — rivals use it too on their death log).
        const bankruptcies = data.school?.lifetimeStats?.bankruptcies || 0;
        if (bankruptcies > 0) parts.push('\uD83D\uDC80 ' + bankruptcies);
      } else {
